...
class LinkedInClient(SocialNetworkClient):
    ...
    def _random_delay(self, min_sec: float = 1.5, max_sec: float = 4.0):
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            # Use Chrome 123 User-Agent for modern footprint
            ...
            stealth_sync(self.page) # Apply stealth
            return True
    ...
    def get_feed_posts(self, limit: int = 10) -> List[SocialPost]:
        ...
        try:
            ...
            # --- STEP 2: Scroll to load more posts (Humanized Bounded Loop) ---
            # Instead of jumping instantly to the bottom, simulate human scrolling.
            # We strictly bound the max scrolls to prevent infinite loop anti-patterns.
            max_scrolls = min(max(8, int(limit)), 15)
            logger.info(f"[LINKEDIN] Simulating human scroll (up to {max_scrolls}x)...")
            
            # Use deterministic UI waiters if available, else fallback to bounded scroll
            stale_count = 0
            prev_post_count = initial_posts
            
            for i in range(max_scrolls):
                # Simulate human reading/scrolling
                # A human scrolls down incrementally
                scroll_depth = random.randint(400, 900)
                self.page.mouse.wheel(0, scroll_depth)
                self._random_delay(0.5, 1.5)
                
                # Second small scroll for momentum
                self.page.mouse.wheel(0, random.randint(100, 300))
                
                # Wait for any new elements to hydrate
                # Per stealth article, varied wait times inside a bounded loop
                if i % 3 == 0:
                    self._random_delay(2, 4) # Pause to "read"
                else:
                    self._random_delay(1, 2)
                
                # Check deterministic signal: amount of posts loaded in DOM
                current_post_count = len(self.page.query_selector_all('[data-view-name="feed-full-update"]'))
                if current_post_count > prev_post_count:
                    prev_post_count = current_post_count
                    stale_count = 0
                else:
                    stale_count += 1
                    if stale_count >= 3:
